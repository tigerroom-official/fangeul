...
## 수익화 규칙 (패널 토론 2026-02-28 확정, 2026-03-04 재검증)
- 플로팅 버블 기본 기능(변환, 키보드, 띄우기)은 **전면 무료** — 페이월 금지
- 멤버 이름 치환(`{{member_name}}`)은 **영구 무료** — 개인화 = 입소문 엔진
- 수익화 대상 = 버블 안 콘텐츠(감성 컬러 팩, 언어별 문구 번들, 테마)
- MVP(Phase 6) = **광고(배너+보상형) + 일회성 IAP** (구독은 Phase 7 이후)
- 감성 컬러 팩: 색상+감성 조합 이름 (퍼플 드림, 골든 아워, 그날 콘서트 하늘 등) — 아이돌/팬덤명 금지, "~팬을 위한" 문구도 금지
- IAP 가격: ₩990(진입) / ₩1,900(단독) / ₩3,900(2팩) + 동남아 현지 가격 필수
- 보상형 광고: 4시간 해금 ("4시간 or 자정 중 빠른 것"), 하루 3회 상한, "팬 패스" 보상 프레이밍
- 배너: 조건부 유지 → 단계적 제거 (보상형 1회 시청 시 세션 배너 제거)
- 전환 퍼널: Day 0~13 허니문(광고 없음, 무제한) → Day 14~ 보상형+슬롯 제한 → IAP 트리거(3회 소진+슬롯 포화)
- 경험 깊이 제한: 즐겨찾기 5슬롯(허니문 무제한, Day 14부터), TTS 5회/일
- D-day 선물 정책: 최애 생일/컴백 당일 24h 전면 해제 (단조증가 타임스탬프 검증 + 중복 방지)
- 수익 구조 목표: 광고 70% + IAP 30% (안정화 후)
- 상세: `docs/discussions/2026-03-04-phase6-monetization-consensus.md`
